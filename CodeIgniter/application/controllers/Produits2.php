...
class Produits2 extends CI_Controller {
    ...
    public function advancedResearch(){
     ...
     public function createProduct(){
        ...
        $data['content'] = 'createProduct';

        $data['additifs'] = $this->Produit->getAdditif();
        $data['marques'] = $this->Produit->getMarque();

        $this->load->vars($data);
        $this->load->view('registerProduct');
    }
    ...
    public function formRegisterProduct(){
       ...
       // ici test preliminaire sue la presence des champs dans la base 
        ...
        if((!empty($nom)) OR (!empty($portion)) OR (!empty($marque))){
			...
			if ( $this->Produit_model->getVerifByBrands($marque )  == true ) {
					echo " marque pas dans la liste du coup => creation " ;
		...
		$this->form_validation->set_rules('nom','marque','required');
