...
function ingredientTree($ingredient, $tree, $indent){
	if(!array_key_exists($ingredient, $tree)){
		return;
	}
	foreach ($tree[$ingredient] as $subIngredient){
		echo $indent.$subIngredient.'<br/>';
		ingredientTree($subIngredient, $tree, $indent.'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;');
	}
}
...
<?php echo print_r($product['firstRankIngredient']) ?>
<br><br>
<?php echo print_r($product['ingredient']) ?>
<br><br>
...
<h2>Ingredients :</h2>
<?php foreach($product['firstRankIngredient'] as $ingredient){
	echo $ingredient.'<br/>';
